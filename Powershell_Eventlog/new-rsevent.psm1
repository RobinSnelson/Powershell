...
.DESCRIPTION
    Writes given events to a supplied eventlog if that log doesn't exist then it creates the named log and then registers the source to it, if the log exists
    it just registers the event source to it, usually the script name. Then continues to write the event as appropriate.
.PARAMETER LogName
    The log name which the event is to be wrote to, defaults to 'Application'
